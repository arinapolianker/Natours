...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiYXJpbmFwb2xpYW5rZXIiLCJhIjoiY2wwOHJ4OXV5MDZhOTNicGhjcWNtczNnOCJ9.4oFSif7kRVbXih8cEIZ7BA';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/arinapolianker/cl09qb62j000914nsw52p8n10',
    scrollZoom: false // center: [-118.249999,34.0499998],
    // zoom: 4
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', 'Acount created successfully!');
              window.setTimeout(function () {
                location.assign('/');
              }, 1000);
            }
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? '/api/users/update-my-password' : '/api/users/update-me';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "".concat(type.toUpperCase(), " updated successfully!")); // Refreshing user photo automatically
              // window.setTimeout(() => {
              //   location.reload();
              // }, 1000);
            }
  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(tourId) {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            stripe = Stripe('pk_test_51KaQmqDtxxn1J05vhCmM8AZDaZtCNwXK9bbDmtrgLNQyYGNnaNNrrGq8dAMJzRK03t1IIZCH7h3xXSqopTuNw2HA003t2oxufz');
            _context.prev = 1;
            _context.next = 4;
...
if (loginForm) loginForm.addEventListener('submit', function (el) {
  el.preventDefault();
...
if (signupForm) signupForm.addEventListener('submit', function (el) {
  el.preventDefault();
...
if (bookBtn) bookBtn.addEventListener('click', function (el) {
  el.target.textContent = 'Processing...';
  var tourId = el.target.dataset.tourId;
  (0, _stripe.bookTour)(tourId);
}); // makes the photo appear right after uploadingit

var userImgEl = document.querySelector('.form__user-photo');
var userImgInputEl = document.querySelector('#picture');

var handleDisplayUserPhoto = function handleDisplayUserPhoto(el) {
  var _el$target$files;

  var imgFile = (_el$target$files = el.target.files) === null || _el$target$files === void 0 ? void 0 : _el$target$files[0];
  if (!(imgFile !== null && imgFile !== void 0 && imgFile.type.startsWith('image/'))) return;
